...
router.get('/', (req, res) => {
  res.json({
    ...
    endpoints: [
      ...
      'POST /api/operations/routes/:routeId/stops/:stopId/complete',
      'POST /api/operations/return-scan',
      'POST /api/operations/returns/process',
      'POST /api/operations/wash-batches',
      'POST /api/operations/wash-batches/:washBatchId/inspect',
      'POST /api/operations/wash-batches/:washBatchId/release',
...
router.get('/households', (req, res) => res.json({ households: operationsService.listHouseholds() }));
...
router.get('/households/:householdId', (req, res) => {
  ...
  if (!household) return res.status(404).json({ error: 'Not Found', message: `Household ${req.params.householdId} not found` });
  res.json(household);
});

router.get('/subscriptions', (req, res) => res.json({ subscriptions: operationsService.listSubscriptions() }));
...
router.get('/pack-plans', (req, res) => res.json({ packPlans: operationsService.listPackPlans() }));

router.get('/pack-plans/:packPlanId', (req, res) => {
  const packPlan = operationsService.getPackPlan(req.params.packPlanId);
  if (!packPlan) return res.status(404).json({ error: 'Not Found', message: `Pack plan ${req.params.packPlanId} not found` });
  res.json(packPlan);
});

router.post('/routes/generate', (req, res) => {
  try {
    const route = operationsService.generateRouteManifest(parseDateOrUndefined(req.body.targetDate), { packPlanId: req.body.packPlanId });
...
router.get('/routes', (req, res) => res.json({ routes: operationsService.listRoutes() }));

router.get('/routes/:routeId', (req, res) => {
  const route = operationsService.getRoute(req.params.routeId);
  if (!route) return res.status(404).json({ error: 'Not Found', message: `Route ${req.params.routeId} not found` });
  res.json(route);
});
...
router.get('/containers', (req, res) => {
  res.json({ containers: operationsService.listContainers({ state: req.query.state, householdId: req.query.householdId }) });
});

router.get('/containers/:containerId', (req, res) => {
  const container = operationsService.getContainer(req.params.containerId);
  if (!container) return res.status(404).json({ error: 'Not Found', message: `Container ${req.params.containerId} not found` });
  res.json(container);
});

router.post('/return-scan', (req, res) => {
  try {
    const result = operationsService.scanReturnedContainer(req.body);
    logger.info(`Scanned returned container ${result.container.containerId}`);
    res.json(result);
  } catch (error) {
    logger.error('Error scanning returned container:', error);
    res.status(400).json({ error: 'Bad Request', message: error.message });
  }
});
...
router.get('/wash-batches', (req, res) => res.json({ washBatches: operationsService.listWashBatches() }));
...
router.get('/billing/accounts', (req, res) => res.json({ accounts: operationsService.listBillingAccounts() }));
...
router.get('/dashboard', (req, res) => res.json(operationsService.getOperationsDashboard()));

if (process.env.ALLDONE_SAMPLE_DATA === 'true') operationsService.seedDemoData();
